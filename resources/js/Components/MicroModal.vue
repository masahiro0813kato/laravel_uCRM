...
<template>
        ...
        <div class="modal__overlay" tabindex="-1" data-micromodal-close>
            <div class="w-2/3 modal__container" role="dialog" aria-modal="true" aria-labelledby="modal-1-title">
                ...
                <main class="modal__content" id="modal-1-content">
                    <div v-if="customers.value" class="w-full mx-auto overflow-auto lg:w-2/3">
                            ...
                            <tbody>
                                <tr v-for="customer in customers.value.data" :key="customer.id">
                                    <td class="px-4 py-3 border-b-2 border-gray-200">
                                        <button @click="setCustomer({ id: customer.id, kana: customer.kana })" type="button" class="text-blue-600">{{ customer.id }}</button>
                                    </td>
                                    <td class="px-4 py-3 border-b-2 border-gray-200">
                                        {{ customer.name }}
                                    </td>
    ...
    <input name="customer" v-model="search" class="w-1/2 px-3 py-1 text-base leading-8 text-gray-700 transition-colors duration-200 ease-in-out bg-gray-100 bg-opacity-50 border border-gray-300 rounded outline-none focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200" />
    <button @click="searchCustomers" type="button" data-micromodal-trigger="modal-1" class="flex px-8 py-2 mx-auto text-lg text-white bg-indigo-500 border-0 rounded focus:outline-none hover:bg-indigo-600">検索する</button>
</template>
@click="toggleStatus"
